const hbs = require('@financial-times/n-express/src/handlebars');
const path = require('path');
const aws  = require('aws-sdk');
const getVersions = require('./versions').get;
...
hbs({
	...
	.then(instance => {
		return Promise.all(
			['wrapper', 'vanilla']
				.map(templateName => {
					return instance.getTemplate(path.join(process.cwd(), `/layout/${templateName}.html`), {
						precompiled: true
					})
						.then(tpl => require('fs').writeFileSync(process.cwd() + `/${templateName}-precompiled.js`, tpl))
				})
		)
	})
// 		.then(templates => {
...
// 			const deploys = templates
// 				.reduce((pairs, tpl) => {
// 					versions.forEach(version => pairs.push({tpl, version}))
// 					return pairs;
// 				}, [])
// 				// .map(({tpl, version}) => {
